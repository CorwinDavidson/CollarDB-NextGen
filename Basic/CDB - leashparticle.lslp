...
TextureMenu(key kIn)
{
    ...
    {
        sName = llList2String(g_textures,iLoop);
        lButtons += [sName];
    } 
...
HandleHTTPDB(integer iSender, integer iNum, string sStr, key kID)
{
    if (iNum == HTTPDB_RESPONSE)
    {
        ...
        string sToken = llGetSubString(sStr, 0, iIndex -1);
        string sValue = llGetSubString(sStr, iIndex + 1, -1);
...
HandleCOMMAND(integer iSender, integer iNum, string sStr, key kID)
{
    if (iNum >= COMMAND_OWNER && iNum <= COMMAND_WEARER)
    {
        if (llToLower(sStr) == llToLower(g_sSubMenu))
        {
            ...
            {
                Notify(kID, "Leash Options can only be changed by Collar Owners.", FALSE);
            }
        }
    }
}
...
default
{
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID)
    {
        ...
        else if (iNum == COMMAND_PARTICLE)
        {
            ...
            else
            {
                ...
                {
                    integer bLeasherIsAv = (integer)llList2String(llParseString2List(sStr, ["|"], [""]), 1);
                    g_bLeasherInRange = TRUE;
                    ...
                    if (bLeasherIsAv)
                    {
                        llListenRemove(g_iLMListener);
                        llListenRemove(g_iLMListernerDetach);
                        if (llGetSubString(sStr, 0, 10)  == "leashhandle")
                        {
    ...
    listen(integer iChannel, string sName, key kID, string sStr)
    {
        if (iChannel == LOCKMEISTER)
        {
            ...
            if (sStr == (string)g_kLeashedTo + "handle detached")
            {
                g_kParticleTarget = g_kLeashedTo;
                StartParticles(g_kParticleTarget);
            }
