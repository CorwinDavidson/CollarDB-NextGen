...
HandleDIALOG(integer iSender, integer iNum, string sStr, key kID)
{
    if (iNum == DIALOG_RESPONSE)
    {
        if (kID==g_kDialogID)
        {
            ...
            if (sMessage == UPMENU)
            {
                llMessageLinked(LINK_SET, MENU_SUBMENU, g_sFontParent, kAv);
            }
            else
            {
                ...
                if (iIndex != -1)
                {
                    ...
                    SetLabel(g_sLabelText);
                    llMessageLinked(LINK_SET, SETTING_SAVE, g_sDesignPrefix + "font=" + (string)g_kFontTexture, NULL_KEY);
                }
                FontMenu(kAv);
            }
        }
...
HandleMENU(integer iSender, integer iNum, string sStr, key kID)
{
    ...
    else if (iNum == MENU_REQUEST)
    {
        ...
        {
            llMessageLinked(LINK_SET, MENU_RESPONSE, g_sFontParent + "|" + g_sFontMenu, NULL_KEY);
        }
    }
/*    
    ...
    if (iNum == COMMAND_OWNER)
    {
        ...
        else if (sCommand == "label")
        {
            ...
            else
            {
                ...
                llMessageLinked(LINK_SET, SETTING_SAVE, "label=" + g_sLabelText, NULL_KEY);
                SetLabel(g_sLabelText);
            }
        }
