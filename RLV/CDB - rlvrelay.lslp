...
//    if ( g_lObjBlackList != [] ) sNewSettings+=",objblacklist:"+llDumpList2String(g_lObjBlackList,"/");
    ...
    llMessageLinked(LINK_SET, SETTING_SAVE, sNewSettings, NULL_KEY);
}
    ...
    if (iNum == DIALOG_RESPONSE)
    {
        ...
        if (~llListFindList([g_kMenuID, g_kMinModeMenuID, g_kListMenuID, g_kListID, g_kAuthMenuID], [kID]))
        {
            if (kID==g_kMenuID || kID == g_kMinModeMenuID)
            {
                ...
                {
                    if (kID == g_kMenuID) llMessageLinked(LINK_SET,MENU_SUBMENU,g_sParentMenu,kAv);
                    else Menu(kAv);
                }
            }
...
default
{
    ...
    {
        if ((iNum >= SETTING_SAVE) && (iNum <= SETTING_RESPONSE))
        {
        ...
        else if ((iNum >= COMMAND_OWNER) && (iNum <= COMMAND_RELAY_SAFEWORD))
        {
