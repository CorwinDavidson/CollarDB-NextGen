...
{ // adds and removes locks for sFolder, which implies saving to central settings and triggering a RLV command (dolockFolder)
    ...
    if ([] != g_lFolderLocks) llMessageLinked(LINK_SET, SETTING_SAVE,  "FolderLocks=" + llDumpList2String(g_lFolderLocks, ","), NULL_KEY);
    else llMessageLinked(LINK_SET, SETTING_DELETE,  "FolderLocks", NULL_KEY);
}
...
{ // adds and removes locks for unshared items, which implies saving to central settings and triggering a RLV command (dolockUnshared)
    ...
    doLockUnshared();
    if (g_iUnsharedLocks) llMessageLinked(LINK_SET, SETTING_SAVE,  "UnsharedLocks=" + (string) g_iUnsharedLocks, NULL_KEY);
    else llMessageLinked(LINK_SET, SETTING_DELETE,  "UnsharedLocks", NULL_KEY);
}
...
SaveFolder(string sStr)
{
    ...
    else
    {
        ...
        g_sCurrentFolder="";
//        if (g_iRemenu) {g_iRemenu=FALSE;  llMessageLinked(LINK_SET, MENU_SUBMENU, "Browse #RLV", g_sMenuUser);}
    }
}
...
HandleDIALOG(integer iSender, integer iNum, string sStr, key kID)
{
    if (iNum == DIALOG_RESPONSE)
    {
        if (kID == g_kRootActionsID)
        {
            ...
            if (sMessage == UPMENU) {llMessageLinked(LINK_SET, MENU_SUBMENU, "Browse #RLV", kAv); return;}
...
HandleMENU(integer iSender, integer iNum, string sStr, key kID)
{
        if (iNum == MENU_REQUEST && sStr == g_sParentMenu)
        {
            ...
            {
                llMessageLinked(LINK_SET, MENU_RESPONSE, g_sParentMenu + "|" + llList2String(g_lChildren,i), NULL_KEY);
            }
        }
        else if (iNum == MENU_SUBMENU && llListFindList(g_lChildren,[sStr]) != -1)
        {
            iPage = 0;
            if (sStr == "Browse #RLV")
            {
    ...
    if (iNum == MENU_SUBMENU)
    {
        ...
        */
         if (llListFindList(g_lChildren,[sStr]) != -1)
        {
            iPage = 0;
            if (sStr == "Browse #RLV")
            {
...
default
{
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID)
    {
        ...
        else if ((iNum >= SETTING_SAVE) && (iNum <= SETTING_DELETE))
        {
